...
split_edge['test']['edge_neg'] = negative_sampling(edge_index, num_nodes=num_nodes_1, num_neg_samples=split_edge['test']['edge'].shape[0], method='dense').T
...
emb2 = torch.nn.Embedding(num_nodes2, node_emb_dim).to(device) # each node has an embedding that has to be learnt
model2 = GNNStack(node_emb_dim, hidden_dim, hidden_dim, num_layers, dropout, emb=True).to(device) # the graph neural network that takes all the node embeddings as inputs to message pass and agregate
link_predictor2 = LinkPredictor(hidden_dim, hidden_dim, 1, num_layers + 1, dropout).to(device) # the MLP that takes embeddings of a pair of nodes and predicts the existence of an edge between them

split_edge2 = {}
split_edge2['test'] = {}
split_edge2['test']['edge'] = edge_index2.clone()[:, test_indices2].T
# split_edge['test']['edge'] = pos_train_edge
split_edge2['test']['edge_neg'] = negative_sampling(edge_index2, num_nodes=num_nodes2, num_neg_samples=split_edge2['test']['edge'].shape[0], method='dense').T
