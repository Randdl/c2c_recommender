...
def run(optim_wd=.0,
    ...
    edge_list = read_from_txt('data/split/item_seller.txt')
    edge_index = torch.tensor([edge_list[0], [i + num_items for i in edge_list[1]]])
    # print(edge_index.shape)

    train_indices = np.loadtxt('data/split/item_seller_train.txt')
    ...
    for e in tqdm(range(epochs)):
        ...
        if (e + 1) % 100 == 0:
            torch.save({
                ...
                'loss': loss,
            }, 'checkpoint/model17_{}.pt'.format(e))
            torch.save(emb.weight, 'checkpoint/emb17_{}.pt'.format(e))
