...
# change item_id to make it different from buyer_id
buyer_item['item_id'] = buyer_item.item_id.apply(lambda x: x + 8922)
buyer_item.head()
...
# check 
num_buyer = len([x for x in G.Nodes() if x.GetId() <= 8921])
num_item = len([x for x in G.Nodes() if x.GetId() > 8921])
print(num_buyer)
print(num_item)
